...
class ConfirmCheckout extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final shipping = transactionType == TransactionType.DELIVERY
        ...
        : 0;
    // Add distance fee only if transaction is DELIVERY
    final distanceFee = transactionType == TransactionType.DELIVERY
        ? AddressInfo.addressDistanceFee
        : 0;

    final total = computeTotalOrder(orders) + shipping + distanceFee;

    final transactionID = generateInvoiceID();
