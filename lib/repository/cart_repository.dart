    ...
    _firebaseFirestore
      ...
      for (var doc in snapshot.docs) {
        ...
        if (productSnapshot.exists) {
          ...
          // Check expiryDate nullability and whether product is not expired
          if (product.isHidden == false &&
              (product.expiryDate == null || !product.expiryDate!.isBefore(DateTime.now())) &&
              cartQuantity != 0) {
            cartWithProductList.add(CartWithProduct(cart: cart, products: product));
          } else {
            // If product is hidden, expired, or out of stock, delete the cart item
            _firebaseFirestore
                .collection(COLLECTION_NAME)
                .doc(cart.id)
                .delete();
