...
import 'package:agritechv2/views/auth/optional_address.dart';
...
class AppRouter {
  final GoRouter router = GoRouter(
    routes: <RouteBase>[
      ...
      GoRoute(
        path: '/optional-address',
        builder: (BuildContext context, GoRouterState state) {
          return const OptionalAddressPage();
        },
      ),
      GoRoute(
        path: '/',
        ...
        routes: <RouteBase>[
          ...
          GoRoute(
            ...
            builder: (BuildContext context, GoRouterState state) {
              ...
              if (state.extra != null) {
                try {
                  addresses = (jsonDecode(state.extra.toString()) as List)
                      .map((item) => Address.fromJson(item))
                      .toList();
                } catch (e) {
                  print("Error decoding address list: $e");
                  // Handle error if necessary, maybe use an empty list or show an error message
                }
              }
